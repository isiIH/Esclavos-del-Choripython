...
export const updateBeca = async(Beca) => {
    const result = await pool.query('UPDATE Becas SET nombre = ?, porcentaje = ?, año = ?, semestre = ?, rut_estudiante = ? WHERE ID = ?',[Beca.nombre, Beca.porcentaje, Beca.año, Beca.semestre, Beca.rut_estudiante, Beca.id])
    return result;
}
