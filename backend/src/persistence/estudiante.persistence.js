...
export const updateEstudiante = async(estudiante) => {
    ...
    const result = await pool.query('UPDATE Estudiante SET nombre = ?, correo = ?, id_programa = ? WHERE rut = ?',[estudiante.nombre, estudiante.correo, estudiante.id_programa, estudiante.rut])
    return result;
}
